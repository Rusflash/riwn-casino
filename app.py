...
def roulette_spin():
    ...
    color = 'red' if result in [1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34,
                                36] else 'black' if result != 0 else 'green'
    ...
    if bet_type == 'number' and result == number:
        ...
        win = True
        win_amount = amount * 2
    elif bet_type == 'dozen' and ((number == 1 and 1 <= result <= 12) or (number == 2 and 13 <= result <= 24) or (
            number == 3 and 25 <= result <= 36)):
        win = True
        win_amount = amount * 3
...
def slots_spin():
    ...
    # Генерируем 9 случайных символов (3x3)
    reels = []
    for i in range(9):
    ...
    # Проверка строк (горизонтальные линии)
    for row in range(3):
        if reels[row * 3] == reels[row * 3 + 1] and reels[row * 3 + 1] == reels[row * 3 + 2]:
            win = True
            win_amount += amount * 5

    # Проверка столбцов (вертикальные линии)
    for col in range(3):
