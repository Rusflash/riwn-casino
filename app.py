...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        c.execute('''INSERT INTO casino_pool (balance, house_edge, updated_at)
                     VALUES (?, ?, datetime('now'))''', (20000, 0.05))
    ...
    if not c.fetchone():
        c.execute('''INSERT INTO users (user_id, username, password, email, balance, is_admin, created_at, last_login)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                  ('admin_001', 'mrdante', hashlib.sha256('1816275'.encode()).hexdigest(), 'admin@ruwin.com', 9999999, 1, datetime.now().isoformat(), datetime.now().isoformat()))
...
def get_pool_balance():
    ...
    row = c.fetchone()
    conn.close()
    return (row[0], row[1]) if row else (20000, 0.05)
...
def update_pool(amount):
    ...
    if row:
        ...
        c.execute("INSERT INTO casino_pool (balance, house_edge, updated_at) VALUES (?, ?, datetime('now'))", (20000 + amount, 0.05))
    conn.commit()
    conn.close()
...
def login():
    try:
        ...
        conn = sqlite3.connect('ruwin.db')
        c = conn.cursor()
        c.execute("SELECT user_id, password, is_admin, is_banned FROM users WHERE username = ?", (username,))
        user = c.fetchone()
        conn.close()
...
def register():
    try:
        ...
        c.execute('''INSERT INTO users (user_id, username, password, email, created_at, last_login)
                     VALUES (?, ?, ?, ?, ?, ?)''',
                  (user_id, username, hashed_password, email, datetime.now().isoformat(), datetime.now().isoformat()))
        conn.commit()
        conn.close()
...
def slots_spin():
    try:
        data = request.get_json(force=True)
        user_id = data.get('session_id', 'test_001')
...
def crash_start():
    try:
        ...
        # Умная экономика: чем больше ставка, тем ниже краш
